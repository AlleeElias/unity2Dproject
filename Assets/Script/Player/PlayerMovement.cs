...
public class PlayerMovement : NetworkBehaviour
{
    ...
    void FixedUpdate()
    {
        if (IsLocalPlayer)
        {
            ...
            if ((Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.DownArrow)) && isGrounded() && horInput == 0)
            {
    ...
    {
        RaycastHit2D raycastHit = Physics2D.BoxCast(boxcol2.bounds.center, boxcol2.bounds.size, 0, Vector2.down, 0.1f,
            groundLayer);
    ...
    private bool onWall()
    {
        RaycastHit2D raycastHit = Physics2D.BoxCast(boxcol2.bounds.center, boxcol2.bounds.size, 0,
